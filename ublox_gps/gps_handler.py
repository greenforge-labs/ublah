...
class GPSHandler:
    ...
    async def _configure_device(self) -> None:
        ...
        try:
            ...
            # Disable RTCM3 output if requested
            if self.config.disable_rtcm3_output:
                await self._disable_rtcm3x_output()
    ...
    async def _disable_rtcm3x_output(self) -> None:
        """Disable RTCM3X output using CFG-USBOUTPROT with fallback to legacy method."""
        try:
            # Try the modern CFG-VALSET approach first
            logger.info("Disabling RTCM3X output on USB interface...")
            
            # Use the static config_set method as per pyubx2 documentation
            # Key ID 0x10780004 = CFG-USBOUTPROT-RTCM3X
            from pyubx2 import SET_LAYER_RAM, SET_LAYER_BBR, SET_LAYER_FLASH, TXN_NONE
            
            layers = SET_LAYER_RAM | SET_LAYER_BBR | SET_LAYER_FLASH  # All layers
            transaction = TXN_NONE
            cfgData = [(0x10780004, 0)]  # CFG-USBOUTPROT-RTCM3X: 0 = disable
            
            cfg_val = UBXMessage.config_set(layers, transaction, cfgData)
            await self._send_ubx_message(cfg_val)
            logger.info("RTCM3X output disabled on USB interface via CFG-USBOUTPROT")
                
        except Exception as e:
            logger.error(f"Failed to disable RTCM3X output via CFG-USBOUTPROT: {e}")
